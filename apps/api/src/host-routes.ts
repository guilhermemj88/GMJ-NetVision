...
const zabbixInput = z.object({
  enabled: z.boolean(),
  hostId: z.string().max(80),
  hostName: z.string().max(255),
  primaryInterfaceId: z.string().max(80),
  ip: z.string().max(45),
});
...
const snmpInput = z.object({
  enabled: z.boolean(),
  version: z.enum(['SNMP_V2C', 'SNMP_V3']),
  host: z.string().max(255),
  port: z.number().int().min(1).max(65_535).default(161),
  community: z.string().min(1).max(1_024).optional(),
  username: z.string().max(128),
  securityLevel: z.enum(['NO_AUTH_NO_PRIV', 'AUTH_NO_PRIV', 'AUTH_PRIV']),
  authProtocol: z.enum(['MD5', 'SHA', 'SHA256']).nullable(),
  authPassword: z.string().min(1).max(1_024).optional(),
  privacyProtocol: z.enum(['DES', 'AES', 'AES256']).nullable(),
  privacyPassword: z.string().min(1).max(1_024).optional(),
  clearCredential: z.boolean().optional(),
});
const basicHostFields = {
  hostname: z.string().min(1).max(255),
  displayName: z.string().min(1).max(120),
  managementIp: z.string().max(45),
  ...
  origin: z.enum(['MANUAL', 'ZABBIX', 'DISCOVERY', 'IMPORTED']),
};
...
const updateHostSchema = z.object({
  hostname: basicHostFields.hostname.optional(),
  displayName: basicHostFields.displayName.optional(),
  managementIp: basicHostFields.managementIp.optional(),
  vendor: basicHostFields.vendor.optional(),
  model: basicHostFields.model.optional(),
  deviceType: basicHostFields.deviceType.optional(),
  site: basicHostFields.site.optional(),
  description: basicHostFields.description.optional(),
  notes: basicHostFields.notes.optional(),
  origin: basicHostFields.origin.optional(),
  zabbix: zabbixInput.optional(),
  ssh: sshInput.optional(),
  snmp: snmpInput.optional(),
});

function safeConnectionResult(
  source: SourceKind,
  state: ConnectionTestResult['state'],
  message: string,
  version?: string,
): ConnectionTestResult {
  return {
...
function filterHosts(
  hosts: ReturnType<DemoMapRepository['listHosts']>,
  query: {
    q?: string | undefined;
    origin?: HostOrigin | undefined;
...
async function zabbixCandidates(
  ...
  if (!zabbix) {
    ...
    return {
      candidates: demoZabbixCandidates(maps.listHosts()),
      version: '6.0.33-demo',
      demoMode: true,
    };
  }
  const [{ version }, candidates] = await Promise.all([
    zabbix.healthcheck(),
    zabbix.getHostCandidates(),
  ]);
...
export function registerHostRoutes(
  ...
  app.get('/api/hosts', async (request) => {
    const query = z
      .object({
        q: z.string().optional(),
        origin: z.enum(['MANUAL', 'ZABBIX', 'DISCOVERY', 'IMPORTED']).optional(),
        source: z.enum(['ZABBIX', 'SSH', 'SNMP']).optional(),
        sort: z.string().optional(),
        direction: z.enum(['asc', 'desc']).optional(),
      })
      .parse(request.query);
    return filterHosts(maps.listHosts(), query);
  });
  ...
  app.post('/api/hosts/import/zabbix', async (request, reply) => {
    const body = z
      .object({ previewId: z.string().min(1), hostIds: z.array(z.string()).min(1) })
      .parse(request.body);
    const interfaces = new Map<string, NetworkInterface[]>();
    if (zabbix) {
      await Promise.all(
        body.hostIds.map(async (hostId) =>
          interfaces.set(hostId, await zabbix.getInterfaces(hostId)),
        ),
      );
    }
  ...
  app.get('/api/hosts/:hostId', async (request, reply) => {
    ...
    return maps.getHost(hostId) ?? reply.code(404).send({ message: 'Host not found' });
  });
  ...
  app.post('/api/hosts', async (request, reply) => {
    try {
      return reply
        .code(201)
        .send(maps.createHost(createHostSchema.parse(request.body) as CreateHostInput));
    } catch (error) {
      if (error instanceof CredentialEncryptionUnavailableError) {
        return reply.code(409).send({
          message: 'Configure CREDENTIAL_ENCRYPTION_KEY antes de salvar credenciais',
        });
      }
      throw error;
    }
  ...
  app.patch('/api/hosts/:hostId', async (request, reply) => {
    const { hostId } = hostIdParams.parse(request.params);
    try {
      const host = maps.updateHost(hostId, updateHostSchema.parse(request.body) as UpdateHostInput);
  ...
  app.get('/api/hosts/:hostId/interfaces', async (request, reply) => {
    const { hostId } = hostIdParams.parse(request.params);
    const host = maps.getHost(hostId);
  ...
  app.post('/api/hosts/:hostId/maps', async (request, reply) => {
    ...
    return result
      ...
      : reply.code(404).send({ message: 'Host or map not found' });
  });
  ...
  app.post('/api/hosts/:hostId/test/:source', async (request, reply) => {
    ...
    const enabled =
      source === 'ZABBIX' ? host.useZabbix : source === 'SSH' ? host.sshEnabled : host.snmpEnabled;
    let result: ConnectionTestResult;
    if (!enabled) {
      result = safeConnectionResult(source, 'DISABLED', 'Fonte desabilitada para este host');
      ...
      result = safeConnectionResult(
        source,
        'FAILED',
        `Transporte ${source} não configurado no servidor`,
      );
    }
    maps.updateSourceHealth(hostId, result);
    return result;
  });
  ...
  app.post('/api/hosts/:hostId/discovery/preview', async (request, reply) => {
    ...
    const host = maps.getHost(hostId);
    if (!host) return reply.code(404).send({ message: 'Host not found' });
    const review = await discovery.discover(host, maps.listHosts());
    let candidates: ZabbixHostCandidate[] = [];
    try {
      candidates = (await zabbixCandidates(maps, zabbix)).candidates;
    } catch {
      review.warnings.push('ZABBIX: correlação indisponível; discovery LLDP preservado');
    }
    const preview = maps.createDiscoveryPreview(hostId, mapId, review, candidates);
    return preview ?? reply.code(409).send({ message: 'O host precisa estar presente no mapa' });
  });
  ...
  app.post('/api/hosts/:hostId/discovery/apply', async (request, reply) => {
        ...
        selections: z
          .array(
            z.object({
              ...
              action: z.enum(['ADD', 'ADD_UNMONITORED', 'LINK_ONLY', 'IGNORE']),
    ...
    const selections = body.selections.map((selection) => ({
      ...
      ...(selection.selectedDeviceId ? { selectedDeviceId: selection.selectedDeviceId } : {}),
    })) satisfies DiscoveryApplySelection[];
