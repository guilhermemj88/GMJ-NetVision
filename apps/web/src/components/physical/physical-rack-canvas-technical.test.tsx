...
function buildFixedDevice(spec: DeviceSpec) {
  ...
  for (const group of spec.groups) {
    for (let index = 1; index <= group.count; index += 1) {
      order += 1;
      const name = group.pattern.replace('{n}', String(index));
      catalogPorts.push({
        name,
        label: name,
        order,
...
describe('visão técnica no rack canvas', () => {
  ...
  it('fora do escopo: equipamento sem desenho técnico continua no modo real', () => {
    const s6730H24 = buildFixedDevice({
      ...S6730,
      catalogKey: 'huawei-s6730-h24x6c',
      model: 'S6730-H24X6C',
      assetId: 'asset-h24',
    });
    const html = renderRack([s6730H24.asset], {
      catalog: [s6730H24.entry],
      visualMode: 'TECHNICAL',
    });
    expect(html).toContain('data-visual="REAL"');
    expect(html).toContain('s6730-h24x6c-front.png');
  });
