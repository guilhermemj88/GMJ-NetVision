...
export function createAutoLayout(map: NetworkMap): Map<string, Position> {
  ...
  map.nodes.forEach((node) =>
    graph.setNode(node.deviceId, { width: NODE_WIDTH, height: NODE_HEIGHT }),
  );
  map.links.forEach((link) => graph.setEdge(link.sourceDeviceId, link.targetDeviceId));
  dagre.layout(graph);

  return new Map(
    map.nodes.map((node) => {
      const position = graph.node(node.deviceId) as { x: number; y: number };
